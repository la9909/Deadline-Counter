...
* [ ] 과제 등록 시 React State에 과제 추가 기능 구현
* [ ] D-Day 계산 함수 유닛 테스트 및 카드 렌더링 연결
* [ ] 정상 등록 시 입력 폼 초기화 로직 구현
...
* [ ] 폼 에러 상태(`errors` object) 관리 구현
* [ ] 12개 예외 케이스 동작 확인 및 UI 에러 텍스트 스타일링
...
* [ ] 5. 마감일을 선택할 수 있다.
* [ ] 6. `과제 등록` 버튼이 존재한다.
* [ ] 7. 정상 입력 후 과제 카드가 생성된다.
* [ ] 8. 카드에 과제명이 표시된다.
* [ ] 9. 카드에 마감일이 표시된다.
* [ ] 10. 카드에 D-Day가 표시된다.
* [ ] 11. 마감일이 오늘이면 `D-Day`로 표시된다.
* [ ] 12. 과거 날짜는 등록할 수 없다.
* [ ] 13. 과제명이 비어 있으면 등록할 수 없다.
* [ ] 14. 공백만 입력한 과제명은 등록할 수 없다.
* [ ] 15. 같은 과제명과 같은 날짜의 중복 등록을 막는다.
* [ ] 16. 연속 클릭으로 동일한 과제가 여러 번 생성되지 않는다.
* [ ] 17. 정상 등록 후 입력값이 초기화된다.
* [ ] 18. 등록 실패 시 정상적으로 입력된 값은 유지된다.
* [ ] 19. 등록된 과제가 없을 때 빈 상태 문구가 표시된다.
* [ ] 20. 로그인 기능이 없다.
* [ ] 21. 결제 기능이 없다.
* [ ] 22. 데이터베이스를 사용하지 않는다.
* [ ] 23. 새로고침 후 데이터 유지 기능을 만들지 않는다.
* [ ] 24. 반응형 UI 레이아웃 검증 (모바일/데스크톱 대응)
* [ ] 25. 코드 리팩토링 및 번들 최적화
